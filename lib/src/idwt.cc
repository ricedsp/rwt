...
void idwt_convolution(data *x_out, size_t lx, data *coeff_low, data *coeff_high, int ncoeff_minus_one, int ncoeff_halved_minus_one, data *x_in_low, data *x_in_high) {
  int k;
  size_t i, j, ind, tj;
  data x0, x1;
 ...
 */
template <typename data>
void idwt_allocate(size_t m, size_t n, int ncoeff, data **x_dummy, data **y_dummy_low, data **y_dummy_high, data **coeff_low, data **coeff_high) {
  *x_dummy      = (data *) rwt_calloc(max(m,n),            sizeof(data));
  *y_dummy_low  = (data *) rwt_calloc(max(m,n)+ncoeff/2-1, sizeof(data));
  *y_dummy_high = (data *) rwt_calloc(max(m,n)+ncoeff/2-1, sizeof(data));
 ...
 */
template <typename data>
void idwt_free(data **x_dummy, data **y_dummy_low, data **y_dummy_high, data **coeff_low, data **coeff_high) {
  rwt_free(*x_dummy);
 ...
 */
template <typename data>
void idwt_coefficients(int ncoeff, data *h, data **coeff_low, data **coeff_high) {
  int i;
  for (i=0; i<ncoeff; i++) {
    (*coeff_low)[i] = h[i];
 ...
 */
template <typename data>
void idwtT(data *x, size_t nrows, size_t ncols, data *h, int ncoeff, int levels, data *y)
{
...
{
    idwtT(x,nrows,ncols,h,ncoeff,levels,y);
}
